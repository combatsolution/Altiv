...
const pieData = [
  { name: 'Augmentation', value: 30, color: '#FFB95A' },
  { name: 'Automation', value: 40, color: '#EF4444' },
  { name: 'Human', value: 30, color: '#84CC16' },
];

const tasksMap = {
  Automation: ['Basic content creation', 'Data reporting', 'Market research', 'Simple assessments'],
  Augmentation: ['Assistive drafting', 'Insight summarization', 'Design suggestions', 'Quality checks'],
  Human: ['Creative brainstorming', 'Complex negotiations', 'Emotional support', 'High-level strategy'],
};

// --- Dynamic Total for center ---
...
const renderActiveShape = (props) => {
  ...
  return (
    <g>
      <text x={cx} y={cy - 10} dy={8} textAnchor="middle" fill="#000" fontSize={16}>
        Total
      </text>
      <text x={cx} y={cy + 10} dy={8} textAnchor="middle" fontSize={20} fontWeight="bold" fill="#000">
        {total}
      </text>
      <Sector
        cx={cx}
        ...
        fill={fill}
      />
      <text x={cx} y={cy + outerRadius + 20} textAnchor="middle" fill={fill} fontSize={14}>
  ...
  return (
    <Box px={{ xs: 2, md: 6 }} py={2} sx={{ position: 'relative' }}>
      <Grid container spacing={4}>
        ...
        >
          {[
            { label: 'Automation', color: '#EF4444', desc: 'AI can do these tasks itself, replacing human work' },
            { label: 'Augmentation', color: '#FFB95A', desc: 'AI works with you, like a smart assistant helping you do better work' },
            { label: 'Human', color: '#84CC16', desc: 'Tasks that AI can’t likely replace' },
          ].map((item, index) => (
            <Box key={index} sx={{ display: 'flex', alignItems: 'center', gap: 1, mt: index === 0 ? 0 : 1 }}>
              <Box sx={{ width: 20, height: 20, bgcolor: item.color }} />
              <Typography sx={{ fontFamily: 'Roboto', fontSize: '12px', color: '#090808' }}>
