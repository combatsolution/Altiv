...
export default function TransformationRoadmap({ data }) {
  const theme = useTheme();
  const isMobile = useMediaQuery(theme.breakpoints.down("sm"));
  const { user } = useAuthContext();
  const isProUser = user?.isPro || false; // ✅ toggle lock overlay

  // ✅ Map API response
  const roadmap = data?.data?.json_schema_data?.roadmap ?? [];

  const formattedData = roadmap.map((yearData, yearIndex) => ({
    id: `Y${yearIndex + 1}`,
    year: yearData.year,
    quarters: yearData.milestones.map((milestone) => ({
      title: milestone.period,
      description: milestone.deliverables?.[0] || "",
      kpi: milestone.deliverables?.[1] || "",
    })),
  }));
                  ...
                  >
                    <Paper
                      ...
                      sx={{
                        ...
                        backgroundColor: "white",
                        position: "relative",
                        minHeight: section.id === "Y1" ? 270 : 200,
                        "&:hover": { boxShadow: 4 },
                      }}
                    >
                      <Typography
