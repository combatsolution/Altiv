...
const SavedJobsPage = () => {
  ...
  useEffect(() => {
    const fetchSavedJobs = async () => {
      try {
        const response = await axiosInstance.post('jobs/saved-jobs');
        if (response.data.success) setJobs(response.data.savedJobs);
          ...
          return (
            <Grid item xs={12} sm={6} md={4} key={job.id}>
              <JobCard>
                ...
                {/* Title & posted */}
                <Stack direction="row" alignItems="center" spacing={1} mt={2} mb={1} flexWrap="wrap">
