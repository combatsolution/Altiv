...
export default function StrategicInsights() {
  ...
  useEffect(() => {
    const data = Company_Fobo_Response?.data?.strategic_insights;
    if (data) {
      const sections = [
        {
          title: "Strengths",
        ...
        },
        {
          ...
          textcolor: "#B45310",
          bgcolor: "#FFFBEB",
          items: data.gaps_and_risks || [],
        },
        {
          title: "Act-Now Levers",
          textcolor: "#4855D1",
          bgcolor: "#EEF2FF",
          items: data.action_levers || [],
        },
      ];
          ...
          <Grid item xs={12} md={4} key={idx}>
            ...
            >
              <CardContent>
                ...
                <ul style={{ paddingLeft: 0, marginTop: "8px" }}>
                  {sec.items.map((text, i) => (
                    <Box key={i} mb={1.2}>
                      <Typography
                        variant="subtitle2"
