...
export default function ResumeTable() {
  ...
  const handleUpload = (e) => {
    ...
    if (file) {
      ...
      // Create a new resume object with minimal info (you can extend it as needed)
      const newResume = {
        id: resumes.length ? resumes[resumes.length - 1].id + 1 : 1,
        name: file.name.replace(/\.[^/.]+$/, ""), // file name without extension as name
        company: '',
        email: '',
        phone: '',
        value: '',
                ...
                {resumes.map((resume, index) => {
                  ...
                  return (
                    <TableRow key={resume.id}>
                      ...
                      <TableCell>{resume.assigned || '-'}</TableCell>
                      <TableCell>
                        <Chip label={resume.status} color={chipColor} size="small" />
                      </TableCell>
                      <TableCell>{resume.source || '-'}</TableCell>
                    </TableRow>
                  );
